...
struct EditingModeBanner: View {
    ...
    var body: some View {
        HStack {
            Image(systemName: isEditingEnabled ? "pencil.circle.fill" : "lock.fill")
                .foregroundColor(isEditingEnabled ? .green : .orange)
            
            VStack(alignment: .leading, spacing: 2) {
                Text(isEditingEnabled ? "editing_mode_active".localized : "read_only_mode".localized)
                    .font(.subheadline.bold())
        ...
        .padding()
        .background((isEditingEnabled ? Color.green : Color.orange).opacity(0.1))
        .cornerRadius(8)
    }
}
